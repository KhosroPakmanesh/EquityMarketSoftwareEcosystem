...
public class OneSideInstitutionalTradingInformationEquality implements IOneSideInstitutionalTradeClearingRule {
    @Override
    public BooleanResultMessage checkRule(List<Trade> trades) {
        ...
        List<UUID> institutionalSideRegisteredCodes = institutionalTrades.stream().
                map(trade -> trade.getBuySide().getTradingInformation().getRegisteredCode()).
                distinct().collect(Collectors.toList());

        if (institutionalSideRegisteredCodes.size() > 1) {
            return new BooleanResultMessage(false,
                    OperationMessage.
                            Create("The registered codes of the" +
