...
public class LimitPriceOrderAlgorithm implements IOrderMatchingAlgorithm {
    ...
    @Inject
    public LimitPriceOrderAlgorithm(ExchangeUnitRepositories exchangeUnitRepositories,
    ...
    public List<Trade> matchBuyLogic(Order inProcessBuyOrder, OrderTypeContainer orderTypeContainer) {
        ...
        for (Order storedSellOrder : orderTypeContainer.getSellOrders()) {
            if (storedSellOrder.getTerm().getPrice() <= inProcessBuyOrder.getTerm().getPrice() &&
                    inProcessBuyOrder.getTerm().getQuantity() > 0) {
    ...
    public List<Trade> matchSellLogic(Order inProcessSellOrder, OrderTypeContainer orderTypeContainer) {
        ...
        List<Trade> trades = new ArrayList<>();
        for (Order storedBuyOrder : orderTypeContainer.getBuyOrders()) {
            if (storedBuyOrder.getTerm().getPrice() >= inProcessSellOrder.getTerm().getPrice() &&
                    inProcessSellOrder.getTerm().getQuantity() > 0) {
