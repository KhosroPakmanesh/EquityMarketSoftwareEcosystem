    ...
    public void runScenario() {
        ...
        InitiateInstitutionalOrderOutputClass initiateInstitutionalOrderOutputClass =
                broker.initiateInstitutionalOrder_UI(
                        ...
                        new Term(20, 1000, "MSFT"), OrderType.MP,
                        emSeco.brokerUnit.core.entities.shared.MoneyTransferMethod.brokerInternalAccount,
                        emSeco.brokerUnit.core.entities.shared.EquityTransferMethod.brokerInternalAccount);
        ...
        BooleanResultMessage depositMoneyResultMessage =
                custodian.depositMoney_UI(clearingBankId, institutionalClient1ClearingBankAccountNumber,
                        ...
                        MoneyTransferMethod.custodianInternalAccount);
        ...
        //---------------------------------------------Assertions------------------------------------------------//
        //-------------------------------------------------------------------------------------------------------//
        assertEquals(4000, retailClient1BrokerBankAccount.getBalance());
        double retailClient1DematAccountQuantity = retailClient1BrokerDematAccount.getInstrumentQuantityPairs()
        ...
        assertEquals(4000, retailClient2BrokerBankAccount.getBalance());
        double retailClient2DematAccountQuantity = retailClient2BrokerDematAccount.getInstrumentQuantityPairs()
                .stream().filter(instrumentQuantityPair -> instrumentQuantityPair.
                        getInstrumentName().equals("MSFT")).findFirst().orElse(null).getQuantity();
        assertEquals(0, retailClient2DematAccountQuantity);

        assertEquals(4000, retailClient3BrokerBankAccount.getBalance());
        double retailClient3DematAccountQuantity = retailClient3BrokerDematAccount.getInstrumentQuantityPairs()
                .stream().filter(instrumentQuantityPair -> instrumentQuantityPair.
                        getInstrumentName().equals("MSFT")).findFirst().orElse(null).getQuantity();
        assertEquals(0, retailClient3DematAccountQuantity);

        assertEquals(4000, retailClient4BrokerBankAccount.getBalance());
        double retailClient4DematAccountQuantity = retailClient4BrokerDematAccount.getInstrumentQuantityPairs()
                .stream().filter(instrumentQuantityPair -> instrumentQuantityPair.
                        getInstrumentName().equals("MSFT")).findFirst().orElse(null).getQuantity();
        assertEquals(0, retailClient4DematAccountQuantity);

        assertEquals(4000, retailClient5BrokerBankAccount.getBalance());
        double retailClient5DematAccountQuantity = retailClient5BrokerDematAccount.getInstrumentQuantityPairs()
                .stream().filter(instrumentQuantityPair -> instrumentQuantityPair.
                        getInstrumentName().equals("MSFT")).findFirst().orElse(null).getQuantity();
        assertEquals(0, retailClient5DematAccountQuantity);


        assertEquals(0, institutionalClient1CustodianBankAccount.getBalance());
        double institutionalClient1DematAccountQuantity =
                institutionalClient1CustodianDematAccount.getInstrumentQuantityPairs()
                        .stream().filter(instrumentQuantityPair -> instrumentQuantityPair.
                        getInstrumentName().equals("MSFT")).findFirst().orElse(null).getQuantity();
